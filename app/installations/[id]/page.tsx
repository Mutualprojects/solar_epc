...
import { DM_Sans } from "next/font/google";
import {
  Wrench, ChevronLeft, ChevronRight, Loader2, MapPin, Phone,
  Camera, Eye, X, CheckCircle, Clock, Save, Lock, Sparkles, Check,
  ChevronDown, Grid3x3, List, ZoomIn, Download, Share2, AlertCircle, Edit3, Award, UploadCloud,
  Database, Layers, Sun, Droplets
} from "lucide-react";
...
export default function InstallationDetailPage({ params }: PageProps) {
  ...
  // ──── Stepper UI state (presentation only) ────
  const [activeStep, setActiveStep] = useState(0); // 0..3 stages, 4 = certificate
  ...
  // ─────────────────── IMAGE GALLERY (unchanged behavior) ───────────────────
  const renderImageGallery = (
    savedImages: string[],
    pendingFiles: PendingFile[],
    isLocked: boolean,
    ...
    if (allImages.length === 0) {
      return (
        <div className="w-full flex flex-col items-center justify-center h-full gap-1.5 py-4">
          <Camera className="w-5 h-5 text-slate-300" />
          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide text-center">
            No images yet
          </span>
        </div>
      );
    ...
    return allImages.map((item, idx) => (
      <div
        key={idx}
        className="relative group/img w-16 h-16 md:w-[72px] md:h-[72px] rounded-xl overflow-hidden border border-slate-200 shadow-sm shrink-0 hover:shadow-md transition-all"
      >
        <img src={getSecureImageUrl(item.url)} alt={`${section}-${idx}`} className="w-full h-full object-cover" />

        {/* Hover Overlay */}
        <div className="absolute inset-0 bg-black/45 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center gap-1.5 backdrop-blur-[1px]">
          <button
            type="button"
            onClick={() => setImageModal({ isOpen: true, imageUrl: item.url, imageIndex: idx })}
            className="p-1.5 bg-white/95 hover:bg-white text-slate-700 rounded-lg transition-all"
          >
          ...
          {!isLocked && (
            <button
              type="button"
              onClick={() => handleDeleteImage(item.index, section, item.isPending)}
              className="p-1.5 bg-rose-500/95 hover:bg-rose-600 text-white rounded-lg transition-all"
            >
        ...
        {/* Pending Badge */}
        {item.isPending && (
          <div className="absolute top-1 right-1 bg-amber-500 text-white text-[7px] font-black px-1.5 py-0.5 rounded shadow">
            NEW
          </div>
        )}
      </div>
  ...
  // ── Live progress reads directly from current status state ──
  const systemCount = inst ? Math.max(1, inst.schools?.no_of_systems || 1) : 1;
  const statusToPctUI = (s: string) => {
    if (s === "Completed") return 100;
    if (s === "In Progress") return 50;
  ...
  // ─────────────────── STAGE CONFIG (wires UI → existing handlers) ───────────────────
  const stages = [
    {
      num: 1, name: "Tank Installation", short: "Tank", key: "tank" as const, icon: Database,
      status: tankStatus, savedStatus: inst?.tank_status || "Pending",
      remarks: tankRemarks, images: tankImages, pending: pendingTankFiles, fileRef: tankFileRef,
      onRemark: setTankRemarks,
      onStatus: (val: string) => { setTankStatus(val); setTankPct(val === "Completed" ? 25 : 0); },
    },
    {
      num: 2, name: "MMS Structure Installation", short: "MMS", key: "mms" as const, icon: Layers,
      status: mmsStatus, savedStatus: inst?.mms_status || "Pending",
      remarks: mmsRemarks, images: mmsImages, pending: pendingMmsFiles, fileRef: mmsFileRef,
      onRemark: setMmsRemarks,
      onStatus: (val: string) => { setMmsStatus(val); setMmsPct(val === "Completed" ? 50 : 0); },
    },
    {
      num: 3, name: "Solar Collectors Installation", short: "Collectors", key: "collectors" as const, icon: Sun,
      status: collStatus, savedStatus: inst?.collectors_status || "Pending",
      remarks: collRemarks, images: collImages, pending: pendingCollFiles, fileRef: collFileRef,
      onRemark: setCollRemarks,
      onStatus: (val: string) => { setCollStatus(val); setCollPct(val === "Completed" ? 75 : 0); },
    },
    {
      num: 4, name: "Plumbing & Electrical Routing", short: "Plumbing", key: "plumbing" as const, icon: Droplets,
      status: plumbStatus, savedStatus: inst?.plumbing_status || "Pending",
      remarks: plumbRemarks, images: plumbImages, pending: pendingPlumbFiles, fileRef: plumbFileRef,
      onRemark: setPlumbRemarks,
      onStatus: (val: string) => { setPlumbStatus(val); setPlumbPct(val === "Completed" ? 100 : 0); },
    },
  ];

  // Step meta for the stepper rail (includes the certificate as the final step)
  const stepMeta = [
    ...stages.map((s) => ({ short: s.short, icon: s.icon, savedStatus: s.savedStatus })),
    { short: "Certificate", icon: Award, savedStatus: completionCertificate ? "Completed" : "Pending" },
  ];

  const stepStateColor = (savedStatus: string) => {
    if (savedStatus === "Completed") return { ring: "border-emerald-500 bg-emerald-500 text-white", text: "text-emerald-700", line: "bg-emerald-500" };
    if (savedStatus === "In Progress") return { ring: "border-amber-400 bg-amber-400 text-white", text: "text-amber-700", line: "bg-amber-400" };
    if (savedStatus === "Suspended") return { ring: "border-rose-400 bg-rose-400 text-white", text: "text-rose-700", line: "bg-rose-400" };
    return { ring: "border-slate-300 bg-white text-slate-400", text: "text-slate-400", line: "bg-slate-200" };
  };

  // ─────────────────── STAGE PANEL RENDERER ───────────────────
  const renderStagePanel = (stage: typeof stages[number]) => {
    const isLocked = stage.savedStatus === "Completed";
    const isInProgress = stage.status === "In Progress";
    const isBeyondApplicable = activeSystem > systemCount;
    const Icon = stage.icon;

    const badgeConfig: Record<string, { label: string; cls: string }> = {
      "Completed": { label: "Completed", cls: "bg-emerald-50 border-emerald-300 text-emerald-700" },
      "In Progress": { label: "In Progress", cls: "bg-amber-50 border-amber-300 text-amber-700" },
      "Suspended": { label: "Suspended", cls: "bg-rose-50 border-rose-300 text-rose-700" },
      "Pending": { label: "Pending", cls: "bg-slate-50 border-slate-200 text-slate-500" },
    };
    const badge = badgeConfig[stage.status] ?? badgeConfig["Pending"];

    return (
      <div key={`${stage.key}-${activeSystem}`} className="step-enter">
        <div className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${isLocked ? "border-emerald-200" : "border-slate-200"}`}>
          {/* Panel header */}
          <div className={`p-5 md:p-6 border-b flex items-center justify-between gap-3 flex-wrap ${isLocked ? "border-emerald-100 bg-gradient-to-r from-emerald-50/70 to-teal-50/40" : "border-slate-100 bg-slate-50/50"}`}>
            <div className="flex items-center gap-3.5 min-w-0">
              <div className={`w-12 h-12 rounded-2xl border flex items-center justify-center shrink-0 shadow-sm ${isLocked ? "bg-emerald-100 border-emerald-300 text-emerald-700" : isInProgress ? "bg-amber-100 border-amber-300 text-amber-700" : "bg-white border-slate-200 text-slate-500"}`}>
                <Icon className="w-5 h-5" />
              </div>
              <div className="min-w-0">
                <h3 className="text-sm md:text-base font-black text-slate-800 tracking-tight leading-tight truncate">
                  {stage.name}
                </h3>
                <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                  Stage {stage.num} of 4 {systemCount > 1 && `· System ${activeSystem}/${systemCount}`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[9px] font-black border uppercase tracking-wider ${badge.cls}`}>
                {stage.status === "Completed" && <Check className="w-3 h-3 mr-1" />}
                {badge.label}
              </span>
              {isLocked && <Lock className="w-4 h-4 text-emerald-600 shrink-0" />}
            </div>
          ...
          <div className="p-4 md:p-5 space-y-4">
            {isBeyondApplicable ? (
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center flex flex-col items-center gap-2">
                ...
                <h4 className="text-xs font-black text-amber-900 uppercase tracking-widest">System Not Applicable</h4>
                <p className="text-[10px] text-amber-700 uppercase font-bold max-w-sm leading-relaxed">
                  This site only has {systemCount} system(s). System {activeSystem} is not required.
                </p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <textarea
                      value={stage.remarks}
                      onChange={(e) => stage.onRemark(e.target.value)}
                      placeholder={`Enter ${stage.name.toLowerCase()} remarks...`}
                      className="bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 rounded-xl p-2.5 text-xs md:text-sm font-medium text-slate-700 outline-none min-h-[80px] placeholder:text-slate-400 transition-all resize-none"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between gap-2">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">
                        Verification Images ({stage.images.length + stage.pending.length})
                      </label>
                      <button
                        type="button"
                        onClick={() => stage.fileRef.current?.click()}
                        disabled={uploadingSection === stage.key}
                        className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white text-[9px] font-black uppercase tracking-wider rounded-lg transition-all flex items-center gap-1.5 cursor-pointer shadow-sm"
                      >
                        {uploadingSection === stage.key ? (
                          <Loader2 className="w-3 h-3 animate-spin" />
                        ) : (
                          <Camera className="w-3 h-3" />
                        )}
                      ...
                      <input
                        type="file"
                        ref={stage.fileRef}
                        onChange={(e) => handleImageUpload(e, stage.key)}
                        className="hidden"
                        multiple
                        accept="image/*"
                      />
                    </div>
                    <div className="bg-slate-50/70 border border-slate-200 rounded-xl p-2.5 min-h-[80px] flex flex-wrap gap-2 items-start content-start overflow-y-auto">
                      {renderImageGallery(stage.images, stage.pending, false, stage.key)}
                    </div>
                  </div>
                </div>

                {/* Status + Save */}
                <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 pt-4 border-t border-slate-100">
                  <div className="flex items-center gap-2.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider whitespace-nowrap">
                      Status
                    </label>
                    <select
                      value={stage.status}
                      onChange={(e) => stage.onStatus(e.target.value)}
                      className={`border focus:ring-4 focus:ring-emerald-500/10 rounded-lg py-2 px-3 text-sm font-bold outline-none transition-all cursor-pointer ${isLocked ? "bg-emerald-50 border-emerald-300 text-emerald-700"
                        : isInProgress ? "bg-amber-50 border-amber-200 text-amber-700"
                          : "bg-slate-50 border-slate-300 text-slate-700"
                        }`}
                    >
                      <option value="Pending">Pending</option>
                  ...
                  <button
                    type="button"
                    onClick={() => saveStageProgress(stage.key)}
                    disabled={savingSection === stage.key || isLocked}
                    title={isLocked ? "Stage already completed and saved. Change status to re-save." : ""}
                    className={`px-5 py-2.5 text-white font-black text-[10px] uppercase tracking-wider rounded-lg shadow-md transition-all flex items-center justify-center gap-2 ${isLocked ? "bg-emerald-300 cursor-not-allowed opacity-70" : "bg-emerald-600 hover:bg-emerald-700 hover:shadow-lg cursor-pointer hover:-translate-y-0.5"
                      } disabled:opacity-60`}
                  >
                    {savingSection === stage.key ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : isLocked ? (
                      <Lock className="w-4 h-4" />
                    ) : (
                    ...
                    {isLocked ? "Saved" : `Save Stage ${stage.num}`}
                  </button>
                </div>
              </>
            )}
  ...
  const renderCertificatePanel = () => (
    <div key="certificate-panel" className="step-enter">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 md:p-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3.5">
            <div className="w-12 h-12 rounded-2xl border border-emerald-200 bg-emerald-50 flex items-center justify-center text-emerald-700 shadow-sm">
              <Award className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-sm md:text-base font-black text-slate-800 tracking-tight leading-tight">
                Completion Certificate
              </h3>
              <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">
                Official Site Verification Document
          ...
          {completionCertificate ? (
            <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-100 border border-emerald-300 text-emerald-800 text-[9px] font-black rounded-full uppercase tracking-wider shadow-sm">
              <Check className="w-3 h-3" /> Uploaded
            </span>
          ) : (
            <span className="inline-flex items-center px-2.5 py-1 bg-slate-100 border border-slate-200 text-slate-500 text-[9px] font-black rounded-full uppercase tracking-wider">
              Pending
            </span>
          )}
        </div>
        ...
        <div className="p-4 md:p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col justify-center space-y-2.5">
              ...
              <p className="text-[9px] text-slate-400 font-semibold uppercase leading-normal">
                Note: This upload is required for administrative sign-off but does not affect the technical installation percentage.
              </p>
            </div>

            <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-xl p-4 bg-slate-50 hover:bg-slate-100/50 transition-colors relative min-h-[110px]">
              {completionCertificate ? (
                <div className="relative group/cert w-full h-[110px] rounded-lg overflow-hidden border border-slate-300 shadow-sm bg-white">
                  <img
                    src={getSecureImageUrl(completionCertificate)}
                    alt="Completion Certificate"
                    className="w-full h-full object-contain"
                  />
                  <div className="absolute inset-0 bg-black/45 opacity-0 group-hover/cert:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-[1px]">
                    <button
                      type="button"
                      onClick={() => setImageModal({ isOpen: true, imageUrl: completionCertificate, imageIndex: 0 })}
                      className="p-2 bg-white/95 hover:bg-white text-slate-700 rounded-lg shadow transition-all cursor-pointer"
                    ...
                    <button
                      type="button"
                      onClick={handleDeleteCertificate}
                      className="p-2 bg-rose-500/95 hover:bg-rose-600 text-white rounded-lg shadow transition-all cursor-pointer"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ...
                <div className="text-center space-y-2.5">
                  ...
                  <button
                    ...
                    onClick={() => certFileRef.current?.click()}
                    disabled={uploadingCert}
                    className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-400 text-white text-[9px] font-black uppercase tracking-wider rounded-lg transition-all inline-flex items-center gap-1.5 cursor-pointer shadow-sm"
                  >
                    {uploadingCert ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Camera className="w-3.5 h-3.5" />}
                    Choose Certificate
                  </button>
                  <input
                    type="file"
  ...
  // ─────────────────── MAIN RENDER ───────────────────
  return (
    <div className={`h-[100vh] w-full bg-gradient-to-br from-slate-50 to-slate-100 flex overflow-hidden ${dmSans.className}`}>
      {/* Sidebar */}
      <div className="hidden lg:flex flex-col">
        <SessionNavBar />
      </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        {toast && (
          <div
            className={`fixed top-4 right-4 z-[99999] px-4 py-3 rounded-xl border shadow-lg flex items-center gap-2 toast-enter ${toast.type === "success" ? "bg-emerald-50 border-emerald-300 text-emerald-800" : "bg-rose-50 border-rose-300 text-rose-800"
              }`}
          >
        ...
        {imageModal.isOpen && imageModal.imageUrl && (
          <div
            className="fixed inset-0 bg-black/80 z-[10000] flex items-center justify-center p-4 modal-fade"
            onClick={() => setImageModal({ isOpen: false, imageUrl: null, imageIndex: 0 })}
          >
                ...
                {Array.from({ length: systemCount }).map((_, idx) => {
                  ...
                  return (
                    <button
                      key={sysNo}
                      onClick={() => handleSwitchSystem(sysNo)}
                      className={`px-2.5 md:px-3 py-1.5 rounded-lg text-[8px] md:text-[9px] font-black uppercase tracking-wider transition-all ${isActive ? "bg-white text-emerald-700 shadow-sm border border-slate-200/80" : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/30"
                        }`}
                    >
                      System {sysNo}
                    </button>
            ...
            {loading ? (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-pulse">
                ...
                <div className="lg:col-span-8 space-y-5">
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-5 h-80" />
                </div>
              </div>
            ) : !inst ? (
              <div className="bg-white rounded-2xl border border-slate-200 p-8 md:p-12 text-center max-w-md mx-auto shadow-sm">
                <div className="w-12 h-12 bg-rose-50 border border-rose-200 rounded-full flex items-center justify-center text-rose-500 font-bold mx-auto mb-4">!</div>
                <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider mb-2">Not Found</h3>
                <p className="text-[10px] text-slate-500 uppercase font-bold leading-relaxed">
                  Installation record could not be loaded.
                </p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-4 space-y-6">
                  {/* Header Card */}
                  <div className="relative bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600"></div>
                    <div className="space-y-4 pt-1">
                      <div>
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          <span className="text-[9px] font-black text-slate-500 uppercase tracking-wider bg-slate-100 border border-slate-200/50 px-2.5 py-1 rounded-lg">
                            {inst.installation_code}
                          </span>
                          <span className={`inline-flex px-2.5 py-1 border rounded-full text-[9px] font-black uppercase tracking-wider ${getStatusBadgeStyles(inst.overall_status)}`}>
                            {inst.overall_status}
                          </span>
                        </div>
                        <h2 className="text-base font-black text-slate-800 uppercase tracking-tight leading-tight mb-1">
                          {inst.schools?.kgbv_name || "UNKNOWN"}
                        </h2>
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block mb-3">
                      Site Execution Level
                    </span>
                    <div className="relative w-32 h-32 mx-auto flex items-center justify-center mb-4">
                      <svg className="w-full h-full transform -rotate-90">
                      ...
                      <div className="absolute flex flex-col items-center justify-center">
                        <span className="text-3xl font-black text-slate-800">{calculatedProgress}%</span>
                        <span className="text-[8px] font-black text-slate-400 uppercase tracking-wider mt-1">Overall</span>
                      </div>
                    </div>
                  ...
                  {/* Milestones Card */}
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block border-b border-slate-100 pb-2 mb-3">
                      Milestones
                    </span>
                    <div className="space-y-2.5">
                      {[
                        { completed: isTankStageCompleted, label: "Tank Installation", step: 0 },
                        { completed: isMmsStageCompleted, label: "MMS Structure", step: 1 },
                        { completed: isCollStageCompleted, label: "Solar Collectors", step: 2 },
                        { completed: isPlumbStageCompleted, label: "Plumbing & Electrical", step: 3 },
                      ].map((item, idx) => (
                        <button
                          key={idx}
                        ...
                        >
                          <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${item.completed ? "bg-emerald-500 border-emerald-600" : "border-slate-300 bg-slate-50 group-hover:border-emerald-300"}`}>
                            {item.completed && <Check className="w-2.5 h-2.5 text-white" />}
                          </div>
                          <span className={`text-[9px] font-bold uppercase tracking-wider transition-colors ${item.completed ? "text-slate-400 line-through" : "text-slate-700 group-hover:text-emerald-700"}`}>
                            {item.label}
                          </span>
                        </button>
                      ))}
                ...
                <div className="lg:col-span-8 space-y-5">
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6 shadow-sm">
                    <div className="flex items-center justify-between mb-5">
                      ...
                      <span className="text-[10px] font-black text-emerald-700 uppercase tracking-wider">
                        Step {activeStep + 1} / {stepMeta.length}
                      </span>
                    </div>

                    <div className="flex items-start">
                      {stepMeta.map((step, idx) => {
                        const colors = stepStateColor(step.savedStatus);
                        const isActive = activeStep === idx;
                        const isDone = step.savedStatus === "Completed";
                        ...
                        return (
                          <div key={idx} className="flex items-start flex-1 last:flex-none">
                            ...
                            >
                              <div
                                className={`relative w-10 h-10 md:w-11 md:h-11 rounded-2xl border-2 flex items-center justify-center transition-all duration-300 ${colors.ring} ${isActive ? "ring-4 ring-emerald-500/15 scale-110 shadow-md" : "group-hover:scale-105"}`}
                              >
                                {isDone ? <Check className="w-5 h-5" /> : <StepIcon className="w-4 h-4 md:w-5 md:h-5" />}
                                {isActive && <span className="absolute -inset-1 rounded-2xl border-2 border-emerald-400/40 step-ping" />}
                              </div>
                              <span className={`text-[8px] md:text-[9px] font-black uppercase tracking-wider text-center transition-colors ${isActive ? "text-emerald-700" : colors.text}`}>
                                {step.short}
                              </span>
                            </button>
                            ...
                            {idx < stepMeta.length - 1 && (
                              <div className="flex-1 h-[3px] mt-5 md:mt-[22px] mx-1.5 rounded-full bg-slate-100 overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-700 ${isDone ? "w-full " + colors.line : "w-0"}`}
                                />
                              </div>
                            )}
                          </div>
                  ...
                  {/* Active step panel */}
                  {activeStep < 4 ? renderStagePanel(stages[activeStep]) : renderCertificatePanel()}
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-[10px] font-black uppercase tracking-wider text-slate-600 shadow-sm hover:border-slate-300 hover:text-slate-800 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    <div className="flex items-center gap-1.5">
                      {stepMeta.map((_, idx) => (
                        <button
                          key={idx}
                          onClick={() => setActiveStep(idx)}
                          className={`h-1.5 rounded-full transition-all ${activeStep === idx ? "w-6 bg-emerald-600" : "w-1.5 bg-slate-300 hover:bg-slate-400"}`}
                        />
                      ))}
                    </div>

                    <button
                      onClick={() => setActiveStep((s) => Math.min(stepMeta.length - 1, s + 1))}
                      disabled={activeStep === stepMeta.length - 1}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-[10px] font-black uppercase tracking-wider shadow-md hover:bg-emerald-700 hover:-translate-y-0.5 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:translate-y-0"
                    >
      ...
      <style>{`
        ...
        .step-enter { animation: stepEnter 0.35s cubic-bezier(0.16, 1, 0.3, 1); }
        ...
        .step-ping { animation: stepPing 1.6s cubic-bezier(0,0,0.2,1) infinite; }
      `}</style>
    </div>
  );
}